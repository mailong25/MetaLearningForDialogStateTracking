...
class Meta(nn.Module):
    ...
    def forward(self, batch_tasks, training = True):
        ...
        for task_id, dataset in enumerate(batch_tasks):
            ...
            ## Compute gradients for original model, which equal fast_model's weights - original's weights
            if training:
            ...
            with torch.no_grad():
                ...
                gate_logits, start_logits, end_logits = fast_model(input_ids, attention_mask, segment_ids, 
                                                                   None, valid_id, slot_embedding)
